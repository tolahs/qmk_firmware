...
def get_QMKCodeDic(code):
    try:
        return QMKCodeDic[code]
    except:
         return code
...
try:
    ...
    print(os.getcwd() + "\n")
    fileInput = open("E:\\tolga\\Repos\\qmk_firmware\\keyboards\\bm40hsrgb\\keymaps\\tolahs\\keymap.c","r")
        ...
        for keymap in keymaps:
            ...
            for i in range(len(keymap)-1):
                ...
                for item in keymap[i+1]:
                    spaces = int(((7 * layout["{0}".format(i)]["{0}".format(itemIndex)]  - 1 - len(get_QMKCodeDic(item)))) /2) + (len(get_QMKCodeDic(item)) - 1 + layout["{0}".format(i)]["{0}".format(itemIndex)])%2
                    for _ in range(spaces):
                        print(' ', end='', file=file)
                    print(get_QMKCodeDic(item), end='',file=file)
                    for _ in range(spaces - (len(get_QMKCodeDic(item)) - 1 + layout["{0}".format(i)]["{0}".format(itemIndex)])%2):
                        print(' ', end='', file=file)
                    print('│', end='',file=file)
                    itemIndex += 1
